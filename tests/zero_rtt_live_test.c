// Live TLS 1.3 0-RTT (early data). With early data enabled, the first request to
// an origin is a fresh full handshake (capturing a 0-RTT-capable ticket); the
// second request writes the GET as early data DURING the handshake and, when the
// server accepts it, completes in 1-RTT with r->early_data set. The control with
// 0-RTT OFF must never report early_data. Probes several origins so the assertion
// targets one that actually supports 0-RTT.
...
// Drive one origin twice with 0-RTT enabled; report whether req2 used early data.
internal B32 probe_origin(EventLoop *loop, const char *url) {
  ...
  fprintf(stderr, "  %-32s a{ok=%d resumed=%d early=%d} b{ok=%d resumed=%d early=%d}\n",
...
int main(void) {
  ...
  // A request LARGER than the server's early-data limit must still complete: the
  // bytes past the cap are queued and flushed as 1-RTT after the handshake (the
  // overflow-resend path). Without it the truncated request would hang — so a
  // watchdog stops the loop and the test fails loudly instead of hanging.
  {
    Client big;
    client_init(&big, &loop, profile_chrome148(), /*verify=*/1);
    client_set_early_data_enabled(&big, 1);
    fetch_once(&big, &loop, "https://cloudflare.com/");  // warm the 0-RTT ticket
    ...
    // request straddles the 0-RTT / 1-RTT boundary) but under the server's header
    // limit, so it should still succeed end-to-end.
    Arena *a = arena_alloc();
    U64 n = 18000;
    ...
    Ctx e;
    MemoryZeroStruct(&e);
    client_request(&big, Method_GET, str8_lit("https://cloudflare.com/"), hdr, 1,
                   0, 0, on_resp, &e);
    loop_run(&loop);
    uv_close((uv_handle_t *)&watchdog, 0);
    loop_run(&loop);  // let the watchdog handle finish closing
    fprintf(stderr, "  big-request over 0-RTT: called=%d got=%d status=%d early=%d\n",
            e.called, e.got, e.status, e.early_data);
    CHECK(e.called);  // it RESOLVED (any status) rather than hanging on truncation
    arena_release(a);
    client_cleanup(&big);
  }
  ...
  // checked under ASan): the connection now always has a handle to close.
  {
    ...
    CHECK(f.called && !f.got);  // the failure was delivered (ok=0), no leak/hang
    client_cleanup(&dns);
  }
  ...
  client_set_resumption_enabled(&off, 1);  // 1-RTT resumption, but NO early data
