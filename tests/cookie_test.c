...
internal void test_scope(Arena *a) {
  ...
  CHECK(
      has(cookie_jar_cookie_header(&jar, a, U("https://a.example.com/"), 1000),
          "sid=1"));
  CHECK(
      !has(cookie_jar_cookie_header(&jar, a, U("https://b.example.com/"), 1000),
...
internal void test_path(Arena *a) {
  CookieJar jar;
  cookie_jar_init(&jar, a);
  cookie_jar_store(&jar, U("https://h.com/api"), str8_lit("p=1; Path=/api"),
                   1000);
  ...
  CHECK(has(cookie_jar_cookie_header(&jar, a, U("https://h.com/api/x"), 1000),
            "p=1"));
  CHECK(
      !has(cookie_jar_cookie_header(&jar, a, U("https://h.com/apidocs"), 1000),
...
internal void test_secure(Arena *a) {
  ...
  CHECK(
      has(cookie_jar_cookie_header(&jar, a, U("https://h.com/"), 1000), "s=1"));
...
internal void test_expiry(Arena *a) {
  ...
  cookie_jar_init(&jar, a);
  U64 now = 1000000000;
  cookie_jar_store(&jar, U("https://h.com/"), str8_lit("m=1; Max-Age=100"),
                   now);
  CHECK(
      has(cookie_jar_cookie_header(&jar, a, U("https://h.com/"), now), "m=1"));
  ...
  CHECK(
      !has(cookie_jar_cookie_header(&jar, a, U("https://h.com/"), now), "e=1"));
  ...
  CHECK(
      has(cookie_jar_cookie_header(&jar, a, U("https://h.com/"), now), "f=1"));
}
...
internal void test_order_and_replace(Arena *a) {
  ...
  cookie_jar_store(&jar, U("https://h.com/a/b"), str8_lit("deep=2; Path=/a/b"),
                   1000);
  String8 hdr =
      cookie_jar_cookie_header(&jar, a, U("https://h.com/a/b/c"), 1000);
  CHECK(has(hdr, "deep=2") && has(hdr, "root=1"));
  // Longest path first: deep (/a/b) before root (/).
  CHECK(str8_find(hdr, str8_lit("deep=2")) <
        str8_find(hdr, str8_lit("root=1")));
...
internal void test_public_suffix(Arena *a) {
  ...
  CHECK(
      !has(cookie_jar_cookie_header(&jar, a, U("https://victim.co.uk/"), 1000),
           "evil=1"));
  ...
  CHECK(!has(
      cookie_jar_cookie_header(&jar, a, U("https://victim.github.io/"), 1000),
      "gh=1"));
  ...
  CHECK(has(cookie_jar_cookie_header(&jar, a, U("https://other.example.co.uk/"),
                                     1000),
            "ok=1"));
  ...
  CHECK(
      !has(cookie_jar_cookie_header(&jar, a, U("https://evil.example/"), 1000),
           "idn=1"));
  ...
  CHECK(has(cookie_jar_cookie_header(&jar, a, U("https://github.io/"), 1000),
            "self=1"));
  CHECK(!has(
      cookie_jar_cookie_header(&jar, a, U("https://user.github.io/"), 1000),
...
internal void test_crumbs(void) {
  ...
  // Values with '=' / '.' / '-' stay intact within their crumb (e.g.
  // cf_clearance).
