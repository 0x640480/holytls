...
// (tests/support/loopback_server), run over BOTH HTTP/2 and HTTP/1.1. The
// server ECHOES: status 200, records the received method, and sends the request
// body straight back. For each transport we verify:
...
static void echo_handler(const LbRequest *req, LbResponse *resp, void *user) {
  (void)user;
  U64 n = req->method.size < sizeof g_seen_method - 1
              ? req->method.size
...
static void run_suite(EventLoop *loop, LbAlpn alpn, HttpVersion ver,
  ...
  // 2) Small POST body round-trips byte-intact.
  {
    ...
    RC rc =
        do_req(loop, &c, Method_POST, url, (const U8 *)small, strlen(small));
    CHECK(rc.ok && rc.status == 200 && rc.body_len == strlen(small) &&
          rc.body_ok);
  }
  ...
  // 3) Large POST (~4MB) round-trips byte-intact (request DATA + egress ring).
  {
    ...
    fprintf(stderr, "  [%s POST 4MB] ok=%d status=%d body=%llu intact=%d\n",
            label, rc.ok, rc.status, (unsigned long long)rc.body_len,
            rc.body_ok);
