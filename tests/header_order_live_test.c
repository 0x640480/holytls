...
// One GET to the oracle; fills cx->recv with the received header names in order.
...
int main(void) {
  ...
  // Pseudo names (always first, in profile order — not affected by the override),
  // then the non-empty default REGULAR headers (the cookie placeholder is omitted).
  const char* pseudo[8];
  int pn = 0;
  ...
  String8 ro[] = {str8_lit("x-raw"), str8_lit("user-agent"), str8_lit("accept")};
