...
// proxy tests used to each carry. A test registers ONE LbHandler; the server owns
// all the TLS/protocol plumbing, accumulates the full request (incl. a multi-MB
// body), calls the handler, and frames the response per the negotiated ALPN.
...
// Queue a raw write on a tcp handle (malloc+copy, freed on completion). No-op on
// zero length or a closing handle.
...
// Bind 127.0.0.1:0, listen, return the OS-assigned port; stores `data` on srv->data.
...
// Start a persistent loopback origin on `loop`; writes the bound port to *out_port
// and returns a handle. Multiple servers may run on one loop.
