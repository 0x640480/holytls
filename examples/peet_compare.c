...
int main(void) {
  ...
  // Override the profile's default headers: send ONLY the headers below — none
  // of the profile's browser headers (sec-ch-ua, sec-fetch-*, accept-encoding).
  // This is what lets us replicate the Go example, which replaces req.Header
  // wholesale. The TLS/HTTP2 fingerprint is unchanged; only the request header
  // set changes.
  ...
  // http.HeaderOrderKey{...}: the explicit wire order. It composes with
  // override mode, so these three are emitted in exactly this order regardless
  // of the array order above (pseudo-headers :method/:authority/:scheme/:path
  // stay first, in the profile's order).
