...
// debugging proxy) so powhttp can capture the EXACT wire bytes holytls sends:
// the real ClientHello (TLS fingerprint), the HTTP/2 SETTINGS/HEADERS frames,
// and the request headers. powhttp terminates (decrypts) the TLS, presenting a
// per-host leaf cert signed by its root CA — so we trust that root with
// client_add_ca_file and keep verification ON, then point the client at the
// proxy with client_set_proxy. holytls's fingerprint is unchanged by the proxy;
// inspect it in powhttp itself (its UI, or the powhttp MCP tools get_tls /
// get_http2_stream / fingerprint).
//
// Start powhttp at 127.0.0.1:8888, then from the repo root:
//   ./build/powhttp_proxy [url] [ca.pem] [proxy-url]
// defaults: https://www.google.com/  "powhttp Root Certificate.pem"
// http://127.0.0.1:8888
#include <stdio.h>
...
int main(int argc, char **argv) {
  ...
  // through an HTTP CONNECT proxy). verify=1 so powhttp's MITM cert is
  // validated.
  Client client;
