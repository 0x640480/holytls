...
// proxy. holytls always speaks TLS to the origin, so a proxy is a
// transport-layer concern: connect to the proxy, negotiate a tunnel (HTTP
// CONNECT or SOCKS5), then run the normal byte-exact target-TLS handshake over
// that tunnel. This header is the pure (no I/O) layer: parsing a proxy URL and
// building/parsing the negotiation bytes. net/connection.c drives the async
// exchange.
...
// Reconstruct a proxy URL "scheme://[user:pass@]host:port" from `p` into
// `arena` (the inverse of proxy_parse; credentials included). {0,0} for
// ProxyType_None.
...
// is present, sets *complete=1 and *status to the parsed HTTP status; returns
// the number of header bytes consumed (so the caller can detect trailing data).
...
// Method-selection reply (2 bytes): *method <- server choice. Returns 1 if
// parsed.
...
U64 proxy_socks5_connect_request(String8 target_host, U16 target_port, U8 *out,
                                 U64 cap);
// Parse the variable-length SOCKS5 reply (CONNECT or UDP ASSOCIATE).
// *complete=1 once the whole reply is present (BND.ADDR length depends on its
// ATYP); *ok <- (REP==0). When non-null, *bnd_atyp / *bnd_addr / *bnd_port
// surface the bound endpoint (the UDP relay for an ASSOCIATE reply); *bnd_addr
// points into `buf`. Returns the reply length when complete, else 0.
...
//   RSV(00 00) FRAG(00) ATYP ADDR PORT. `atyp` is 0x01 (IPv4, addrlen 4) or
//   0x04
// (IPv6, addrlen 16); `addr` is the target's resolved address bytes. Returns
// the header length written, 0 if it doesn't fit.
