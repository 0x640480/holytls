...
// lets repeat connections to a host skip uv_getaddrinfo. Fingerprint-neutral: the
// cached address is exactly what getaddrinfo would have returned, so only the
// lookup latency is saved. Single-threaded (driven on the owning loop thread, like
// the rest of the Client) — no locking. Fixed-array + linear scan, matching the
// other per-origin caches; DNS lookups are per-connection, so capacity (not lookup
// speed) is what this trades off.
...
// Keyed by host (DNS is host->IP; port is orthogonal): one entry serves any port,
// and the caller resets the port via dns_sockaddr_set_port on a hit. ttl_ms == 0
// disables the cache (every lookup misses, every store is a no-op).
...
// On a live (unexpired) hit, copy the address into *out / *out_len and return 1.
// Miss / disabled / expired -> 0 (an expired entry is freed in passing).
...
// existing entry for the host; when full, reuses an expired slot, else evicts the
