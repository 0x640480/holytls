...
// + secure + not-expired, longest-path-first). Arena-backed; the caller
// supplies "now" as epoch seconds (wall clock) so the jar has no time
// dependency itself.
...
// now_epoch = wall-clock seconds (time(NULL)). Silently ignores malformed
// input, a Domain that doesn't domain-match the request host, or a bare-TLD
// Domain.
...
// re-seed a jar from already-parsed cookies (and by cookie_jar_store
// internally).
...
// Build the "n1=v1; n2=v2" Cookie header value for `request` into `out`
// (longest path-match first). Returns str8_zero() when nothing matches.
