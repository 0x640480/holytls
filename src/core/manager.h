// Manager — a thread-safe registry of lightweight Sessions by string ID. It owns
// session STATE, not transport (the caller passes a shared Client to
...
// Pin + return the session for id (refcount++). 0 if unknown. Pair with release.
