...
// mirroring the h2 session shape so the client drives it the same way. It owns
// no transport: the serialized request goes to a send callback, received
// plaintext is fed to h1_session_recv, and the response is delivered through a
// callback. The request is serialized verbatim (caller hands wire-cased header
// names, in order) — Chrome/wreq HTTP/1.1 faithfulness lives in the caller's
// casing. Responses are parsed with h2o/picohttpparser.
#ifndef HOLYTLS_H1_H
#define HOLYTLS_H1_H
...
// Feed received plaintext. Returns bytes consumed (>=0) or <0 on protocol
// error.
S64 h1_session_recv(H1Session *s, const U8 *data, U64 len);

// Peer closed the connection (clean EOF). Finalizes + delivers a
// close-delimited body; no-op if already delivered or if the response was
// incomplete (the caller then reports a connection-closed error).
