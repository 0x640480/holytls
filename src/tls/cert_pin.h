...
// configure_ssl ONLY when the connection's host is pinned. Non-pinned hosts keep
// the library's default certificate verification entirely untouched — pinning
// never weakens the common path.
...
enum { CERT_PIN_MAX = 32 };  // total pins across all hosts (backup pins included)
...
// (HPKP / Chrome DevTools format). Returns 1 on success; 0 if the base64 does not
// decode to exactly 32 bytes, an argument is null, or the table is full.
B32 cert_pin_add(CertPinStore *s, const char *host, const char *spki_b64,
                 B32 include_subdomains);

// 1 if any pin applies to `host` (exact, or a parent pinned with include_subdomains).
...
// Attach `store` to `ctx` so per-connection verification can find it. Idempotent.
...
// verify enforcing the pin (trust-on-pin). Called from configure_ssl; a no-op for
// non-pinned hosts (their verification is left exactly as the default path).
