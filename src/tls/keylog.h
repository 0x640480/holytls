...
//
// A single SSL_CTX callback covers BOTH TLS-over-TCP and QUIC: ngtcp2 derives
// the QUIC keys from these same TLS secrets, and Wireshark reads the identical
// file to decrypt either transport — so no separate ngtcp2 hook is needed.
//
// The destination is process-global (one file, matching SSLKEYLOGFILE
// semantics); writes are mutex-serialized and line-flushed so concurrent
// clients / loops never interleave a line. OFF unless a destination is set, and
// a no-op observer that cannot alter any handshake bytes.
...
// destination set for the process wins (like SSLKEYLOGFILE: one file per
// process). Returns 1 if a destination is now open. Thread-safe; idempotent.
B32 keylog_open(const char *path);

// Open from $SSLKEYLOGFILE if set (no-op otherwise). Idempotent; build_ctx
// calls it.
